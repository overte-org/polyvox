...
// Test our ability to modify the behaviour of the
// MarchingCubesSurfaceExtractor. This simple example only modifies the
// threshold (and actually this can be achieved by passing a parameter to the
// constructor of the DefaultMarchingCubesController) but you could implement
// custom behaviour in the other members if you wanted too. Actually, it's not
// clear if this ability is really useful because I can't think what you'd
// modify apart from the threshold but the ability is at least available. Also,
// the DefaultMarchingCubesController is templatised whereas this exmple shows
// that controllers don't have to be.
...
public:
  ...
  float convertToMaterial(float /*voxel*/) { return 1; }

  float getThreshold(void) { return 50.0f; }
};

// These 'writeDensityValueToVoxel' functions provide a unified interface for
// writting densities to primative and class voxel types. They are conceptually
// the inverse of the 'convertToDensity' function used by the
// MarchingCubesSurfaceExtractor. They probably shouldn't be part of PolyVox,
// but they might be usful to other tests so we cold move them into a 'Tests.h'
...
void testForType(SurfaceMesh<PositionMaterialNormal> &result) {
  ...
  for (int32_t z = 0; z < uVolumeSideLength; z++) {
    for (int32_t y = 0; y < uVolumeSideLength; y++) {
      for (int32_t x = 0; x < uVolumeSideLength; x++) {
        ...
        writeMaterialValueToVoxel<VoxelType>(
  ...
  MarchingCubesSurfaceExtractor<SimpleVolume<VoxelType>> extractor(
      &volData, volData.getEnclosingRegion(), &result, controller);
  extractor.execute();
}
...
void testCustomController(SurfaceMesh<PositionMaterialNormal> &result) {
  ...
  MarchingCubesSurfaceExtractor<SimpleVolume<float>,
                                CustomMarchingCubesController>
      extractor(&volData, volData.getEnclosingRegion(), &result, controller);
  extractor.execute();
}
...
void TestSurfaceExtractor::testExecute() {
  ...
  QBENCHMARK { testForType<int8_t>(mesh); }
