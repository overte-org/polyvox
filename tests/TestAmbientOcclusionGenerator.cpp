...
public:
    bool operator()(uint8_t voxel) const { return voxel == 0; }
};
...
void TestAmbientOcclusionGenerator::testExecute() {
    ...
    // Create empty volume
    SimpleVolume<uint8_t> volData(Region(Vector3DInt32(0, 0, 0), Vector3DInt32(g_uVolumeSideLength - 1, g_uVolumeSideLength - 1,
    ...
    // Create an array to store the result. Array can be smaller than the volume
    ...
    QBENCHMARK {
        calculateAmbientOcclusion(&volData, &ambientOcclusionResult, volData.getEnclosingRegion(), 32.0f, 255,
                                  isVoxelTransparent);
    }

    // Check the results by sampling along a line though the centre of the volume.
    // Because of the two walls we added, samples in the middle are darker than
    // those at the edge.
    ...
    // Just run a quick test to make sure that it compiles when taking a function
    // pointer
    calculateAmbientOcclusion(&volData, &ambientOcclusionResult, volData.getEnclosingRegion(), 32.0f, 8,
                              &isVoxelTransparentFunction);

    // Also test it using a lambda
    // calculateAmbientOcclusion(&volData, &ambientOcclusionResult,
    // volData.getEnclosingRegion(), 32.0f, 8, [](uint8_t voxel){return voxel ==
    // 0;});
}
