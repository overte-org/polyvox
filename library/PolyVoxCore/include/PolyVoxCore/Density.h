...
// able to declare an instance of VoxelType::MaterialType without knowing that
// VoxelType doesn't actually have a material.
...
public:
    ...
    bool operator==(const Density& rhs) const { return (m_uDensity == rhs.m_uDensity); };
    ...
    // intuitive way. In particular the ability to add and subtract densities is
    // important in order to apply an averaging filter. The ability to divide by
    // an integer is also needed for this same purpose.
...
// many purposes (including most games) but 16-bit and float types do have uses
// particularly in medical/scientific visualisation.
...
public:
    ...
    DensityType convertToDensity(Density<Type> voxel) { return voxel.getDensity(); }
