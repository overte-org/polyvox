...
LowPassFilter<SrcVolumeType, DstVolumeType, AccumulationType>::LowPassFilter(SrcVolumeType* pVolSrc,
                                                                             Region regSrc,
...
void LowPassFilter<SrcVolumeType, DstVolumeType, AccumulationType>::executeSAT() {
  ...
  currentVal = static_cast<AccumulationType>(m_pVolSrc->getVoxelAt(x,y,z));
    ...
    for (int32_t iDstZ = v3dDstLowerCorner.getZ(), iSrcZ = v3dSrcLowerCorner.getZ(); iDstZ <= v3dDstUpperCorner.getZ();
         iDstZ++, iSrcZ++) {
        for (int32_t iDstY = v3dDstLowerCorner.getY(), iSrcY = v3dSrcLowerCorner.getY(); iDstY <= v3dDstUpperCorner.getY();
             iDstY++, iSrcY++) {
            for (int32_t iDstX = v3dDstLowerCorner.getX(), iSrcX = v3dSrcLowerCorner.getX(); iDstX <= v3dDstUpperCorner.getX();
                 iDstX++, iSrcX++) {
