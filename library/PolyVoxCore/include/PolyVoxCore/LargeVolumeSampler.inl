...
// #define BORDER_HIGH(x) (( x % mVolume->m_uBlockSideLength) !=
// mVolume->m_uBlockSideLength - 1)
...
template <typename VoxelType> LargeVolume<VoxelType>::Sampler::~Sampler() {}

template <typename VoxelType>
typename LargeVolume<VoxelType>::Sampler &
LargeVolume<VoxelType>::Sampler::operator=(
    const typename LargeVolume<VoxelType>::Sampler &rhs) {
  if (this == &rhs) {
    return *this;
  }
...
template <typename VoxelType>
VoxelType
LargeVolume<VoxelType>::Sampler::getSubSampledVoxel(uint8_t uLevel) const {
  if (uLevel == 0) {
    return getVoxel();
  } else if (uLevel == 1) {
...
template <typename VoxelType>
void LargeVolume<VoxelType>::Sampler::setPosition(
    const Vector3DInt32 &v3dNewPos) {
  setPosition(v3dNewPos.getX(), v3dNewPos.getY(), v3dNewPos.getZ());
}

template <typename VoxelType>
void LargeVolume<VoxelType>::Sampler::setPosition(int32_t xPos, int32_t yPos,
                                                  int32_t zPos) {
  ...
  if (this->mVolume->m_regValidRegionInBlocks.containsPoint(
    ...
    Block<VoxelType> *pUncompressedCurrentBlock =
