...
namespace PolyVox {
template <typename VertexType> SurfaceMesh<VertexType>::SurfaceMesh() {
  m_iTimeStamp = -1;
}
...
uint32_t SurfaceMesh<VertexType>::getNoOfNonUniformTrianges(void) const {
  ...
  for (uint32_t i = 0; i < m_vecTriangleIndices.size() - 2; i += 3) {
    if ((m_vecVertices[m_vecTriangleIndices[i]].getMaterial() ==
         m_vecVertices[m_vecTriangleIndices[i + 1]].getMaterial()) &&
        (m_vecVertices[m_vecTriangleIndices[i]].getMaterial() ==
         m_vecVertices[m_vecTriangleIndices[i + 2]].getMaterial())) {
    } else {
      result++;
    }
  }
...
SurfaceMesh<VertexType>::getVertices(void) const {
  return m_vecVertices;
}

template <typename VertexType>
void SurfaceMesh<VertexType>::addTriangle(uint32_t index0, uint32_t index1,
                                          uint32_t index2) {
...
template <typename VertexType>
void SurfaceMesh<VertexType>::addTriangleCubic(uint32_t index0, uint32_t index1,
                                               uint32_t index2) {
  // Make sure the specified indices correspond to valid vertices.
  assert(index0 < m_vecVertices.size());
  assert(index1 < m_vecVertices.size());
...
template <typename VertexType> void SurfaceMesh<VertexType>::clear(void) {
  m_vecVertices.clear();
...
/// Note: This function can cause lighting discontinuities accross region
/// boundaries.
////////////////////////////////////////////////////////////////////////////////
/*template <typename VertexType>
void SurfaceMesh<VertexType>::sumNearbyNormals(bool bNormaliseResult)
{
        if(m_vecVertices.size() == 0) //FIXME - I don't think we should need
this test, but I have seen crashes otherwise...
        {
                return;
        }
        ...
        //Initialise all normals to zero. Should be ok as the vector should
store all elements contiguously. memset(&summedNormals[0], 0,
summedNormals.size() * sizeof(Vector3DFloat));

        for(vector<uint32_t>::iterator iterIndex = m_vecTriangleIndices.begin();
iterIndex != m_vecTriangleIndices.end();)
        {
                ...
                Vector3DFloat sumOfNormals = v0.getNormal() + v1.getNormal() +
v2.getNormal();
...
/*template <typename VertexType>
void SurfaceMesh<VertexType>::generateAveragedFaceNormals(bool bNormalise, bool
bIncludeEdgeVertices)
{
        Vector3DFloat offset =
static_cast<Vector3DFloat>(m_Region.getLowerCorner());

        //Initially zero the normals
        for(vector<PositionMaterialNormal>::iterator iterVertex =
m_vecVertices.begin(); iterVertex != m_vecVertices.end(); iterVertex++)
        {
                if(m_Region.containsPoint(iterVertex->getPosition() + offset,
0.001))
                {
...
iterIndex != m_vecTriangleIndices.end();)
        {
                ...
                Vector3DFloat triangleNormal =
(v1.getPosition()-v0.getPosition()).cross(v2.getPosition()-v0.getPosition());
...
/*template <typename VertexType>
polyvox_shared_ptr< SurfaceMesh<VertexType> >
SurfaceMesh<VertexType>::extractSubset(std::set<uint8_t> setMaterials)
{
        polyvox_shared_ptr< SurfaceMesh<VertexType> > result(new
...
this test, but I have seen crashes otherwise...
        ...
        {
                //If we have done progressive LOD then it's too late to split
into subsets. return result;
        }
        ...
        for(uint32_t triCt = 0; triCt < m_vecTriangleIndices.size(); triCt += 3)
        {
                ...
                PositionMaterialNormal& v0 =
m_vecVertices[m_vecTriangleIndices[triCt]]; PositionMaterialNormal& v1 =
m_vecVertices[m_vecTriangleIndices[triCt + 1]]; PositionMaterialNormal& v2 =
m_vecVertices[m_vecTriangleIndices[triCt + 2]];

                if(
                        (setMaterials.find(v0.getMaterial()) !=
setMaterials.end()) || (setMaterials.find(v1.getMaterial()) !=
setMaterials.end()) || (setMaterials.find(v2.getMaterial()) !=
setMaterials.end()))
                {
                        uint32_t i0;
                        if(indexMap[m_vecTriangleIndices[triCt]] == -1)
                        {
                                indexMap[m_vecTriangleIndices[triCt]] =
result->addVertex(v0);
                        }
                        ...
                        {
                                indexMap[m_vecTriangleIndices[triCt+1]] =
result->addVertex(v1);
                        }
                        ...
                        {
                                indexMap[m_vecTriangleIndices[triCt+2]] =
result->addVertex(v2);
                        }
...
template <typename VertexType>
polyvox_shared_ptr<SurfaceMesh<VertexType>>
extractSubset(SurfaceMesh<VertexType> &inputMesh,
              std::set<uint8_t> setMaterials) {
  polyvox_shared_ptr<SurfaceMesh<VertexType>> result(
      new SurfaceMesh<VertexType>);

  result->m_Region = inputMesh.m_Region;

  if (inputMesh.m_vecVertices.size() ==
      0) // FIXME - I don't think we should need this test, but I have seen
         // crashes otherwise...
  {
    return result;
  }
  ...
  for (uint32_t triCt = 0; triCt < inputMesh.m_vecTriangleIndices.size();
       triCt += 3) {

    VertexType &v0 =
        inputMesh.m_vecVertices[inputMesh.m_vecTriangleIndices[triCt]];
    VertexType &v1 =
        inputMesh.m_vecVertices[inputMesh.m_vecTriangleIndices[triCt + 1]];
    VertexType &v2 =
        inputMesh.m_vecVertices[inputMesh.m_vecTriangleIndices[triCt + 2]];

    if ((setMaterials.find(v0.getMaterial()) != setMaterials.end()) ||
      ...
      if (indexMap[inputMesh.m_vecTriangleIndices[triCt + 1]] == -1) {
        indexMap[inputMesh.m_vecTriangleIndices[triCt + 1]] =
            result->addVertex(v1);
      }
      ...
      if (indexMap[inputMesh.m_vecTriangleIndices[triCt + 2]] == -1) {
        indexMap[inputMesh.m_vecTriangleIndices[triCt + 2]] =
            result->addVertex(v2);
      }
