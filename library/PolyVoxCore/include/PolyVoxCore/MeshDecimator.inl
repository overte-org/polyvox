...
MeshDecimator<VertexType>::MeshDecimator(const SurfaceMesh<VertexType>* pInputMesh,
                                         SurfaceMesh<VertexType>* pOutputMesh,
                                         float fEdgeCollapseThreshold) :
    m_pInputMesh(pInputMesh), m_pOutputMesh(pOutputMesh), m_fMinDotProductForCollapse(fEdgeCollapseThreshold) {
...
bool MeshDecimator<VertexType>::collapseChangesFaceNormals(uint32_t uSrc, uint32_t uDst, float fThreshold) {
    ...
    for (std::vector<uint32_t>::iterator triIter = triangles.begin(); triIter != triangles.end(); triIter++) {
        ...
        const Vector3DFloat& v0OldPos =
            m_pOutputMesh->m_vecVertices[vertexMapper[v0Old]].getPosition();  // Note: we need the vertex mapper here. These
...
template <typename VertexType>
bool MeshDecimator<VertexType>::isSubset(std::bitset<RFF_NO_OF_REGION_FACE_FLAGS> a,
