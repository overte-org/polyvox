...
/// \param[out] pOutputMesh A pointer to where the result should be stored. Any
/// existing contents will be deleted.
/// \param fEdgeCollapseThreshold This is only use in the case of a Marching
/// Cubes surface and controls how close two normals must be to collapse. The
/// dot product between the normals is computed and compared to this threshold.
/// A threshold of 1.0 means nothing will collapse, a threshold of 0.0 means
/// everything will collapse.
////////////////////////////////////////////////////////////////////////////////
template <typename VertexType>
MeshDecimator<VertexType>::MeshDecimator(
    const SurfaceMesh<VertexType> *pInputMesh,
    SurfaceMesh<VertexType> *pOutputMesh, float fEdgeCollapseThreshold)
    : m_pInputMesh(pInputMesh), m_pOutputMesh(pOutputMesh),
      m_fMinDotProductForCollapse(fEdgeCollapseThreshold) {
  *m_pOutputMesh = *m_pInputMesh;
}

template <typename VertexType> void MeshDecimator<VertexType>::execute() {
  // Sanity check.
  if ((m_pOutputMesh->m_vecVertices.empty()) ||
      (m_pOutputMesh->m_vecTriangleIndices.empty())) {
    return;
  }
  ...
  do {
    ...
    if (noOfEdgesCollapsed > 0) {
      // Build the connectivity data for the next pass. If this is slow, then
      // look at adjusting it (based on vertex mapper?) rather than bulding from
      // scratch.
      buildConnectivityData();
    }
...
void MeshDecimator<VertexType>::buildConnectivityData(void) {
  ...
  for (uint32_t triCt = 0; triCt < m_vecTriangles.size(); triCt++) {
    m_vecTriangles[triCt].v0 =
        m_pOutputMesh->m_vecTriangleIndices[triCt * 3 + 0];
    m_vecTriangles[triCt].v1 =
        m_pOutputMesh->m_vecTriangleIndices[triCt * 3 + 1];
    m_vecTriangles[triCt].v2 =
        m_pOutputMesh->m_vecTriangleIndices[triCt * 3 + 2];

    Vector3DFloat v0Pos =
        m_pOutputMesh->m_vecVertices[m_vecTriangles[triCt].v0].position;
    Vector3DFloat v1Pos =
        m_pOutputMesh->m_vecVertices[m_vecTriangles[triCt].v1].position;
    Vector3DFloat v2Pos =
        m_pOutputMesh->m_vecVertices[m_vecTriangles[triCt].v2].position;
...
template <typename VertexType>
uint32_t MeshDecimator<VertexType>::performDecimationPass(
  ...
  for (uint32_t ctIter = 0; ctIter < m_vecTriangles.size(); ctIter++) {
    if (attemptEdgeCollapse(m_vecTriangles[ctIter].v0,
                            m_vecTriangles[ctIter].v1)) {
      ++noOfEdgesCollapsed;
    }

    if (attemptEdgeCollapse(m_vecTriangles[ctIter].v1,
                            m_vecTriangles[ctIter].v2)) {
      ++noOfEdgesCollapsed;
    }

    if (attemptEdgeCollapse(m_vecTriangles[ctIter].v2,
                            m_vecTriangles[ctIter].v0)) {
      ++noOfEdgesCollapsed;
    }
  }
  ...
  if (noOfEdgesCollapsed > 0) {
    // Fix up the indices
    for (uint32_t triCt = 0; triCt < m_pOutputMesh->m_vecTriangleIndices.size();
         triCt++) {
      ...
      if (before != after) {
        m_pOutputMesh->m_vecTriangleIndices[triCt] =
            vertexMapper[m_pOutputMesh->m_vecTriangleIndices[triCt]];
      }
    }
  }
...
template <typename VertexType>
bool MeshDecimator<VertexType>::attemptEdgeCollapse(uint32_t uSrc,
                                                    uint32_t uDst) {
  // A vertex will be locked if it has already been involved in a collapse this
  // pass.
...
template <typename VertexType>
bool MeshDecimator<VertexType>::canCollapseRegionEdge(uint32_t uSrc,
                                                      uint32_t uDst) {
  ...
  if (isSubset(m_vecInitialVertexMetadata[uSrc].isOnRegionFace,
               m_vecInitialVertexMetadata[uDst].isOnRegionFace) == false) {
    return false;
  }
  ...
  if (m_vecInitialVertexMetadata[uSrc].normal.dot(
          m_vecInitialVertexMetadata[uDst].normal) < 0.999f) {
    return false;
  }
...
template <typename VertexType>
bool MeshDecimator<VertexType>::canCollapseMaterialEdge(uint32_t /*uSrc*/,
                                                        uint32_t /*uDst*/) {
  return false;
}
...
template <typename VertexType>
bool MeshDecimator<VertexType>::collapseChangesFaceNormals(uint32_t uSrc,
                                                           uint32_t uDst,
                                                           float fThreshold) {
  bool faceFlipped = false;
  std::vector<uint32_t> &triangles = trianglesUsingVertex[uSrc];

  for (std::vector<uint32_t>::iterator triIter = triangles.begin();
    ...
    const Vector3DFloat &v0OldPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v0Old]]
            .getPosition(); // Note: we need the vertex mapper here. These
                            // neighbouring vertices may have been moved.
    const Vector3DFloat &v1OldPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v1Old]].getPosition();
    const Vector3DFloat &v2OldPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v2Old]].getPosition();

    const Vector3DFloat &v0NewPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v0New]].getPosition();
    const Vector3DFloat &v1NewPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v1New]].getPosition();
    const Vector3DFloat &v2NewPos =
        m_pOutputMesh->m_vecVertices[vertexMapper[v2New]].getPosition();
    ...
    // NOTE: I don't think we should be using the threshold here, we're just
    // checking for a complete face flip
...
// Returns true if every bit which is set in 'a' is also set in 'b'. The reverse
// does not need to be true.
template <typename VertexType>
bool MeshDecimator<VertexType>::isSubset(
    std::bitset<RFF_NO_OF_REGION_FACE_FLAGS> a,
