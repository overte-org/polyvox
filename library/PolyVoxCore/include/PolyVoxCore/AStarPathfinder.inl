...
/// inside the volume and if its density is below that returned by the voxel's
/// getDensity() function.
...
bool aStarDefaultVoxelValidator(const VolumeType* volData, const Vector3DInt32& v3dPos) {
    // Voxels are considered valid candidates for the path if they are inside the
    // volume...
...
////////////////////////////////////////////////////////////////////////////////
template <typename VolumeType>
AStarPathfinder<VolumeType>::AStarPathfinder(const AStarPathfinderParams<VolumeType>& params) : m_params(params) {
}
...
void AStarPathfinder<VolumeType>::execute() {
    ...
    AllNodesContainer::iterator startNode =
        allNodes.insert(Node(m_params.start.getX(), m_params.start.getY(), m_params.start.getZ())).first;
    AllNodesContainer::iterator endNode =
        allNodes.insert(Node(m_params.end.getX(), m_params.end.getY(), m_params.end.getZ())).first;

    // Regarding the const_cast - normally you should not modify an object which
    // is in an sdt::set. The reason is that objects in a set are stored sorted in
    // a tree so they can be accessed quickly, and changing the object directly
    // can break the sorting. However, in our case we have provided a custom sort
    // operator for the set which we know only uses the position to sort. Hence we
    // can safely modify other properties of the object while it is in the set.
    ...
    if ((openNodes.empty()) || (openNodes.getFirst() != endNode)) {
        ...
        // Regarding the const_cast - normally you should not modify an object which
        // is in an sdt::set. The reason is that objects in a set are stored sorted
        // in a tree so they can be accessed quickly, and changing the object
        // directly can break the sorting. However, in our case we have provided a
        // custom sort operator for the set which we know only uses the position to
        // sort. Hence we can safely modify other properties of the object while it
        // is in the set.
...
void AStarPathfinder<VolumeType>::processNeighbour(const Vector3DInt32& neighbourPos, float neighbourGVal) {
    ...
    std::pair<AllNodesContainer::iterator, bool> insertResult =
        allNodes.insert(Node(neighbourPos.getX(), neighbourPos.getY(), neighbourPos.getZ()));
...
float AStarPathfinder<VolumeType>::EighteenConnectedCost(const Vector3DInt32& a, const Vector3DInt32& b) {
    ...
    // just letting it fall through to the 6-connected case. This means 'h' will
    // be bigger than it should be, resulting in a faster path which may not
