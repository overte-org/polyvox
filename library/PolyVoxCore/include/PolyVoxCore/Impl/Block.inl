...
Block<VoxelType>::Block(uint16_t uSideLength)
    : m_tUncompressedData(0), m_uSideLength(0), m_uSideLengthPower(0),
      m_bIsCompressed(true), m_bIsUncompressedDataModified(true) {
  if (uSideLength != 0) {
...
template <typename VoxelType>
VoxelType Block<VoxelType>::getVoxelAt(uint16_t uXPos, uint16_t uYPos,
                                       uint16_t uZPos) const {
  ...
  return m_tUncompressedData[uXPos + uYPos * m_uSideLength +
...
template <typename VoxelType>
void Block<VoxelType>::setVoxelAt(uint16_t uXPos, uint16_t uYPos,
                                  uint16_t uZPos, VoxelType tValue) {
  assert(uXPos < m_uSideLength);
  assert(uYPos < m_uSideLength);
  assert(uZPos < m_uSideLength);
  ...
  m_tUncompressedData[uXPos + uYPos * m_uSideLength +
                      uZPos * m_uSideLength * m_uSideLength] = tValue;

  m_bIsUncompressedDataModified = true;
}

template <typename VoxelType>
void Block<VoxelType>::setVoxelAt(const Vector3DUint16 &v3dPos,
                                  VoxelType tValue) {
  setVoxelAt(v3dPos.getX(), v3dPos.getY(), v3dPos.getZ(), tValue);
}

template <typename VoxelType> void Block<VoxelType>::fill(VoxelType tValue) {
  if (!m_bIsCompressed) {
    // The memset *may* be faster than the std::fill(), but it doesn't compile
    // nicely in 64-bit mode as casting the pointer to an int causes a loss of
    // precision.
...
uint32_t Block<VoxelType>::calculateSizeInBytes(void) {
  ...
  uSizeInBytes +=
      m_vecCompressedData.capacity() * sizeof(RunlengthEntry<uint16_t>);
  return uSizeInBytes;
}

template <typename VoxelType> void Block<VoxelType>::compress(void) {
...
template <typename VoxelType> void Block<VoxelType>::uncompress(void) {
