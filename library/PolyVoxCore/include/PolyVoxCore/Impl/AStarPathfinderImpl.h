...
enum Connectivity {
  ...
  EighteenConnected,
  /// Each voxel has 26 neighbours, which are those sharing a face, edge, or
  /// corner.
  TwentySixConnected
};
...
struct Node {
  Node(int x, int y, int z)
      : gVal(std::numeric_limits<float>::quiet_NaN()) // Initilise with NaNs so
                                                      // that we will
        ,
        hVal(std::numeric_limits<float>::quiet_NaN()) // know if we forget to
                                                      // set these properly.
        ,
        parent(0) {
    position.setX(x);
  ...
  bool operator==(const Node &rhs) const { return position == rhs.position; }
  ...
  float f(void) const { return gVal + hVal; }
};
...
public:
  bool operator()(const AllNodesContainer::iterator &lhs,
                  const AllNodesContainer::iterator &rhs) const {
    return (&(*lhs)) < (&(*rhs));
  }
};
...
public:
  void clear(void) { open.clear(); }

  bool empty(void) const { return open.empty(); }
  ...
  AllNodesContainer::iterator getFirst(void) { return open[0]; }
  ...
  iterator begin(void) { return open.begin(); }

  iterator end(void) { return open.end(); }

  iterator find(AllNodesContainer::iterator node) {
    std::vector<AllNodesContainer::iterator>::iterator openIter =
...
class ClosedNodesContainer {
public:
  typedef std::set<AllNodesContainer::iterator,
                   AllNodesContainerIteratorComparator>::iterator iterator;

public:
  void clear(void) { closed.clear(); }

  void insert(AllNodesContainer::iterator node) { closed.insert(node); }

  void remove(iterator iterToRemove) { closed.erase(iterToRemove); }

  iterator begin(void) { return closed.begin(); }

  iterator end(void) { return closed.end(); }
...
private:
  std::set<AllNodesContainer::iterator, AllNodesContainerIteratorComparator>
      closed;
};

// bool operator<(const AllNodesContainer::iterator& lhs, const
// AllNodesContainer::iterator& rhs);
} // namespace PolyVox
