...
// Included here rather than in the .h because it refers to LargeVolume (avoids
// forward declaration)
#include "PolyVoxCore/ConstVolumeProxy.h"

namespace PolyVox {
////////////////////////////////////////////////////////////////////////////////
/// When construncting a very large volume you need to be prepared to handle the
/// scenario where there is so much data that PolyVox cannot fit it all in
/// memory. When PolyVox runs out of memory, it identifies the least recently
/// used data and hands it back to the application via a callback function. It
/// is then the responsibility of the application to store this data until
/// PolyVox needs it again (as signalled by another callback function). Please
/// see the LargeVolume  class documentation for a full description of this
/// process and the required function signatures. If you really don't want to
/// handle these events then you can provide null pointers here, in which case
/// the data will be discarded and/or filled with default values.
/// \param dataRequiredHandler The callback function which will be called when
/// PolyVox tries to use data which is not currently in memory.
/// \param dataOverflowHandler The callback function which will be called when
/// PolyVox has too much data and needs to remove some from memory.
/// \param uBlockSideLength The size of the blocks making up the volume. Small
/// blocks will compress/decompress faster, but there will also be more of them
...
/// required to provide the call back function (see the other LargeVolume
/// constructor).
/// \param regValid Specifies the minimum and maximum valid voxel positions.
/// \param dataRequiredHandler The callback function which will be called when
/// PolyVox tries to use data which is not currently in momory.
/// \param dataOverflowHandler The callback function which will be called when
/// PolyVox has too much data and needs to remove some from memory.
/// \param bPagingEnabled Controls whether or not paging is enabled for this
/// LargeVolume.
/// \param uBlockSideLength The size of the blocks making up the volume. Small
/// blocks will compress/decompress faster, but there will also be more of them
/// meaning voxel access could be slower.
...
/// expensive, and we want to prevent users from doing it by accident (such as
/// when passing them as paramenters to functions). That said, there are times
/// when you really do want to make a copy of a volume and in this case you
/// should look at the Volumeresampler.
///
/// \sa VolumeResampler
////////////////////////////////////////////////////////////////////////////////
template <typename VoxelType>
...
////////////////////////////////////////////////////////////////////////////////
/// The border value is returned whenever an atempt is made to read a voxel
/// which is outside the extents of the volume.
...
////////////////////////////////////////////////////////////////////////////////
/// Increasing the size of the block cache will increase memory but may improve
/// performance. You may want to set this to a large value (e.g. 1024) when you
/// are first loading your volume data and then set it to a smaller value
/// (e.g.64) for general processing.
/// \param uMaxNumberOfUncompressedBlocks The number of blocks for which
...
////////////////////////////////////////////////////////////////////////////////
/// Increasing the number of blocks in memory causes fewer calls to
/// dataRequiredHandler()/dataOverflowHandler()
...
////////////////////////////////////////////////////////////////////////////////
/// Note that if MaxNumberOfBlocksInMemory is not large enough to support the
...
////////////////////////////////////////////////////////////////////////////////
/// Removes all voxels in the specified Region from memory, and calls
/// dataOverflowHandler() to ensure the application has a chance to store the
/// data. It is possible that there are no voxels loaded in the Region, in which
/// case the function will have no effect.
////////////////////////////////////////////////////////////////////////////////
template <typename VoxelType>
