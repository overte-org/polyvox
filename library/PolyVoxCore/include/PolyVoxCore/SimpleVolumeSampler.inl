...
#define BORDER_HIGH(x) \
    ((((x + 1) >> this->mVolume->m_uBlockSideLengthPower) << this->mVolume->m_uBlockSideLengthPower) != (x + 1))
...
template <typename VoxelType>
typename SimpleVolume<VoxelType>::Sampler& SimpleVolume<VoxelType>::Sampler::operator=(
        ...
        for (uint8_t z = 0; z < uSize; ++z) {
            for (uint8_t y = 0; y < uSize; ++y) {
                for (uint8_t x = 0; x < uSize; ++x) {
                    tValue = (std::min)(tValue, this->mVolume->getVoxelAt(this->mXPosInVolume + x, this->mYPosInVolume + y,
