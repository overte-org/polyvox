...
#define BORDER_LOW(x)                                                          \
  (((x >> this->mVolume->m_uBlockSideLengthPower)                              \
    << this->mVolume->m_uBlockSideLengthPower) != x)
#define BORDER_HIGH(x)                                                         \
  ((((x + 1) >> this->mVolume->m_uBlockSideLengthPower)                        \
    << this->mVolume->m_uBlockSideLengthPower) != (x + 1))
// #define BORDER_LOW(x) (( x % this->mVolume->m_uBlockSideLength) != 0)
// #define BORDER_HIGH(x) (( x % this->mVolume->m_uBlockSideLength) !=
// this->mVolume->m_uBlockSideLength - 1)
...
template <typename VoxelType> SimpleVolume<VoxelType>::Sampler::~Sampler() {}

template <typename VoxelType>
typename SimpleVolume<VoxelType>::Sampler &
SimpleVolume<VoxelType>::Sampler::operator=(
    const typename SimpleVolume<VoxelType>::Sampler &rhs) {
  if (this == &rhs) {
    return *this;
  }
...
template <typename VoxelType>
VoxelType
SimpleVolume<VoxelType>::Sampler::getSubSampledVoxel(uint8_t uLevel) const {
  if (uLevel == 0) {
    return getVoxel();
  } else if (uLevel == 1) {
    ...
    for (uint8_t z = 0; z < uSize; ++z) {
      for (uint8_t y = 0; y < uSize; ++y) {
        for (uint8_t x = 0; x < uSize; ++x) {
          tValue = (std::min)(
              tValue, this->mVolume->getVoxelAt(this->mXPosInVolume + x,
                                                this->mYPosInVolume + y,
                                                this->mZPosInVolume + z));
        }
      }
    }
    return tValue;
 ...
 */
template <typename VoxelType>
void SimpleVolume<VoxelType>::Sampler::setPosition(
 ...
 */
template <typename VoxelType>
void SimpleVolume<VoxelType>::Sampler::setPosition(int32_t xPos, int32_t yPos,
                                                   int32_t zPos) {
  this->mXPosInVolume = xPos;
  this->mYPosInVolume = yPos;
  this->mZPosInVolume = zPos;

  const int32_t uXBlock =
      this->mXPosInVolume >> this->mVolume->m_uBlockSideLengthPower;
  const int32_t uYBlock =
      this->mYPosInVolume >> this->mVolume->m_uBlockSideLengthPower;
  const int32_t uZBlock =
      this->mZPosInVolume >> this->mVolume->m_uBlockSideLengthPower;

  const uint16_t uXPosInBlock = static_cast<uint16_t>(
      this->mXPosInVolume -
      (uXBlock << this->mVolume->m_uBlockSideLengthPower));
  const uint16_t uYPosInBlock = static_cast<uint16_t>(
      this->mYPosInVolume -
      (uYBlock << this->mVolume->m_uBlockSideLengthPower));
  const uint16_t uZPosInBlock = static_cast<uint16_t>(
      this->mZPosInVolume -
      (uZBlock << this->mVolume->m_uBlockSideLengthPower));

  const uint32_t uVoxelIndexInBlock =
      uXPosInBlock + uYPosInBlock * this->mVolume->m_uBlockSideLength +
      uZPosInBlock * this->mVolume->m_uBlockSideLength *
          this->mVolume->m_uBlockSideLength;

  if (this->mVolume->m_regValidRegionInBlocks.containsPoint(
          Vector3DInt32(uXBlock, uYBlock, uZBlock))) {
    Block *pUncompressedCurrentBlock =
        this->mVolume->getUncompressedBlock(uXBlock, uYBlock, uZBlock);

    mCurrentVoxel =
        pUncompressedCurrentBlock->m_tUncompressedData + uVoxelIndexInBlock;
  } else {
    mCurrentVoxel =
        this->mVolume->m_pUncompressedBorderData + uVoxelIndexInBlock;
  }
}
...
bool SimpleVolume<VoxelType>::Sampler::setVoxel(VoxelType tValue) {
  VoxelType *pBorderDataEndPlusOne = this->mVolume->m_pUncompressedBorderData +
                                     this->mVolume->m_uNoOfVoxelsPerBlock;

  // Make sure we're not trying to write to the border data
  if ((mCurrentVoxel < this->mVolume->m_pUncompressedBorderData) ||
      (mCurrentVoxel >= pBorderDataEndPlusOne)) {
...
void SimpleVolume<VoxelType>::Sampler::movePositiveX(void) {
  ...
  if ((++this->mXPosInVolume) % this->mVolume->m_uBlockSideLength != 0) {
    ...
    // We've hit the block boundary. Just calling setPosition() is the easiest
    // way to resolve this.
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel1nx1ny1nz(void) const {
  if (BORDER_LOW(this->mXPosInVolume) && BORDER_LOW(this->mYPosInVolume) &&
      BORDER_LOW(this->mZPosInVolume)) {
    return *(mCurrentVoxel - 1 - this->mVolume->m_uBlockSideLength -
             this->mVolume->m_uBlockSideLength *
                 this->mVolume->m_uBlockSideLength);
  }
  return this->mVolume->getVoxelAt(this->mXPosInVolume - 1,
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel1nx1ny0pz(void) const {
  ...
  return this->mVolume->getVoxelAt(
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel1nx1ny1pz(void) const {
  if (BORDER_LOW(this->mXPosInVolume) && BORDER_LOW(this->mYPosInVolume) &&
      BORDER_HIGH(this->mZPosInVolume)) {
    return *(mCurrentVoxel - 1 - this->mVolume->m_uBlockSideLength +
             this->mVolume->m_uBlockSideLength *
                 this->mVolume->m_uBlockSideLength);
  }
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel1nx0py1nz(void) const {
  ...
  return this->mVolume->getVoxelAt(this->mXPosInVolume - 1, this->mYPosInVolume,
                                   this->mZPosInVolume - 1);
}
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel0px1ny1nz(void) const {
  ...
  return this->mVolume->getVoxelAt(this->mXPosInVolume, this->mYPosInVolume - 1,
                                   this->mZPosInVolume - 1);
}
...
VoxelType SimpleVolume<VoxelType>::Sampler::peekVoxel0px0py1nz(void) const {
  ...
  return this->mVolume->getVoxelAt(this->mXPosInVolume, this->mYPosInVolume,
                                   this->mZPosInVolume - 1);
}
