...
// 'Uniform Grid Intersection Test' in 'Real Time Collision Detection'. The
...
//  handle negative line coordinates the computation of these variables should
...
raycastWithEndpoints(VolumeType *volData, const Vector3DFloat &v3dStart,
  ...
  // defined between voxels. We actually want to define the areas as being
  // centered on voxels (as this is what the CubicSurfaceExtractor generates).
  // We add 0.5 here to adjust for this.
 ...
 * provide a camera position and view direction for picking purposes, but the
 * view direction is usually normalised (i.e. of length one). If you use this
 * view direction directly you will only iterate over a single voxel and won't
 * find what you are looking for. Instead you must scale the direction vector so
 * that it's length represents the maximum distance over which you want the ray
 * to be cast.
 *
 * \param volData The volume to pass the ray though
 * \param v3dStart The start position in the volume
