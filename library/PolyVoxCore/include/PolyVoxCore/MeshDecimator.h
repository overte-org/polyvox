...
////////////////////////////////////////////////////////////////////////////////
/// Meshes generated by the PolyVox surface extractors typically have a very
/// high number of triangles in them. This can pose difficulties both for the
/// rendering storage of such meshes. The MeshDecimator provides a way of
/// reducing the triangle count with minimal visual effect.
///
/// The MeshDecimator is based on the principle of edge collapse, and currently
/// works with meshes generated by the MarchingCubesSurfaceExtractor or
/// CubicSurfaceExtractor. It does not work with meshes generated by the
/// CubicSurfaceExtractorWithNormals, although this may be addressed in the
/// future. The algorithm iterates over each pair of connected vertices in the
/// mesh and attemps to determine if they can be collapsed into a single vertex.
///
/// The main criteria used in deciding whether two vertices can collapse is
/// whether they have the same normal. In the case of the cubic surfaces the
/// normals must be exactly the same, whereas in the case of the Marching Cubes
/// surfaces a threshold is used to determine whether two normals are 'close
/// enough'. Additional constraints apply to vertices which lie on the edges of
/// regions or on the boundary between two regions - these vertices are much
...
/// \endcode
///
/// The above applies for a cubic mesh, for a Marching Cubes mesh you need to
/// parametise the MeshDecimator and resulting SurfaceMesh on the
