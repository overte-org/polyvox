...
example, when running the surface extractors it is unlikely that you will want
to run it on the whole volume at once, as this will give a very large mesh which
may be too much to render. Instead you will probably want to run a surface
extractor a number of times on different parts of the volume, there by giving a
number of meshes which can be culled and rendered seperately.

The Region class is used to define these parts (regions) of the volume.
Essentially it consists of an upper and lower bound which specify the range of
voxels positions considered to be part of the region. Note that these bounds are
<em>inclusive</em>. The class also provides functions for modifying the regions
in a variety of ways.

\Note The dimensions of a region can be measured either in voxels or in cells.
See the manual for more information about these definitions.
*/
#ifdef SWIG
	class Region
#else
...
public:
  ...
  Region(int32_t iLowerX, int32_t iLowerY, int32_t iLowerZ, int32_t iUpperX,
         int32_t iUpperY, int32_t iUpperZ);
  ...
  // FIXME - Don't like these. Make containsPoint take flags indicating which
  // axes to check?
...
private:
  ...
  // FIXME - This variable is unused, but without it the OpenGL example crashes
  // in release mode when the volume size is 128^3 and the level of detail is 2.
  // Very strange, but consistant. Presubablly some kind of alignment issue? It
  // started after this class was changed to use int16's rather than int32's. To
  // be investigated.
  uint8_t dummy;
};
} // namespace PolyVox
