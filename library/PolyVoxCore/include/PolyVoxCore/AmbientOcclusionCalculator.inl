...
void calculateAmbientOcclusion(
  ...
  // code below makes sure they are different for different regions which helps
  // reduce tiling patterns in the results.
  uRandomUnitVectorIndex += region.getLowerCorner().getX() +
                            region.getLowerCorner().getY() +
                            region.getLowerCorner().getZ();
  uRandomVectorIndex += region.getLowerCorner().getX() +
                        region.getLowerCorner().getY() +
                        region.getLowerCorner().getZ();
  ...
  // This loop iterates over the bottom-lower-left voxel in each of the cells in
  // the output array
  for (uint16_t z = region.getLowerCorner().getZ();
       z <= region.getUpperCorner().getZ(); z += iRatioZ) {
    for (uint16_t y = region.getLowerCorner().getY();
         y <= region.getUpperCorner().getY(); y += iRatioY) {
      for (uint16_t x = region.getLowerCorner().getX();
           x <= region.getUpperCorner().getX(); x += iRatioX) {
        ...
        for (int ct = 0; ct < uNoOfSamplesPerOutputElement; ct++) {
          // We take a random vector with components going from -1 to 1 and
          // scale it to go from -halfRatio to +halfRatio. This jitter value
          // moves our sample point from the centre of the array cell to
          // somewhere else in the array cell
          Vector3DFloat v3dJitter =
              randomVectors[(uRandomVectorIndex += (++uIndexIncreament)) %
                            1019]; // Prime number helps avoid repetition on
                                   // successive loops.
          ...
          RaycastResult result =
              raycastWithDirection(volInput, v3dRayStart, v3dRayDirection,
                                   ambientOcclusionCalculatorRaycastCallback);
        ...
        if (uNoOfSamplesPerOutputElement == 0) {
          // The user might request zero samples (I've done this in the past
          // while debugging - I don't want to wait for ambient occlusion but I
          // do want as valid result for rendering). Avoid the divide by zero.
          fVisibility = 1.0f;
        } else {
          fVisibility = static_cast<float>(uVisibleDirections) /
