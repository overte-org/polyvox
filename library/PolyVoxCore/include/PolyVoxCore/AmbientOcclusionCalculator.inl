        ...
        for (uint16_t y = region.getLowerCorner().getY(); y <= region.getUpperCorner().getY(); y += iRatioY) {
            for (uint16_t x = region.getLowerCorner().getX(); x <= region.getUpperCorner().getX(); x += iRatioX) {
                ...
                for (int ct = 0; ct < uNoOfSamplesPerOutputElement; ct++) {
                    ...
                    Vector3DFloat v3dJitter = randomVectors[(uRandomVectorIndex += (++uIndexIncreament)) %
                    ...
                    Vector3DFloat v3dRayDirection =
                        randomUnitVectors[(uRandomUnitVectorIndex += (++uIndexIncreament)) % 1021];  // Different prime number.
                    v3dRayDirection *= fRayLength;

                    AmbientOcclusionCalculatorRaycastCallback<IsVoxelTransparentCallback>
                        ambientOcclusionCalculatorRaycastCallback(isVoxelTransparentCallback);
                    RaycastResult result =
                        raycastWithDirection(volInput, v3dRayStart, v3dRayDirection, ambientOcclusionCalculatorRaycastCallback);
