...
#define BORDER_LOWX(val)                                                       \
  (val > this->mVolume->getEnclosingRegion().getLowerCorner().getX())
#define BORDER_HIGHX(val)                                                      \
  (val < this->mVolume->getEnclosingRegion().getUpperCorner().getX())
#define BORDER_LOWY(val)                                                       \
  (val > this->mVolume->getEnclosingRegion().getLowerCorner().getY())
#define BORDER_HIGHY(val)                                                      \
  (val < this->mVolume->getEnclosingRegion().getUpperCorner().getY())
#define BORDER_LOWZ(val)                                                       \
  (val > this->mVolume->getEnclosingRegion().getLowerCorner().getZ())
#define BORDER_HIGHZ(val)                                                      \
  (val < this->mVolume->getEnclosingRegion().getUpperCorner().getZ())
...
RawVolume<VoxelType>::Sampler::Sampler(RawVolume<VoxelType> *volume)
    : BaseVolume<VoxelType>::template Sampler<RawVolume<VoxelType>>(volume),
      mCurrentVoxel(0), m_bIsCurrentPositionValidInX(false),
      m_bIsCurrentPositionValidInY(false), m_bIsCurrentPositionValidInZ(false) {
}

template <typename VoxelType> RawVolume<VoxelType>::Sampler::~Sampler() {}
...
template <typename VoxelType>
void RawVolume<VoxelType>::Sampler::setPosition(int32_t xPos, int32_t yPos,
                                                int32_t zPos) {
...
bool RawVolume<VoxelType>::Sampler::setVoxel(VoxelType tValue) {
  ...
  // this->mVolume->getBorderValue();
  if (m_bIsCurrentPositionValidInX && m_bIsCurrentPositionValidInY &&
      m_bIsCurrentPositionValidInZ) {
...
void RawVolume<VoxelType>::Sampler::movePositiveX(void) {
  ...
  ++mCurrentVoxel;
  m_bIsCurrentPositionValidInX =
      this->mVolume->getEnclosingRegion().containsPointInX(this->mXPosInVolume);
}
...
void RawVolume<VoxelType>::Sampler::movePositiveY(void) {
  ...
  m_bIsCurrentPositionValidInY =
      this->mVolume->getEnclosingRegion().containsPointInY(this->mYPosInVolume);
}
...
void RawVolume<VoxelType>::Sampler::movePositiveZ(void) {
  ...
  m_bIsCurrentPositionValidInZ =
      this->mVolume->getEnclosingRegion().containsPointInZ(this->mZPosInVolume);
}
...
VoxelType RawVolume<VoxelType>::Sampler::peekVoxel1nx1ny1nz(void) const {
  if (BORDER_LOWX(this->mXPosInVolume) && BORDER_LOWY(this->mYPosInVolume) &&
      BORDER_LOWZ(this->mZPosInVolume)) {
...
VoxelType RawVolume<VoxelType>::Sampler::peekVoxel1nx1ny1pz(void) const {
  if (BORDER_LOWX(this->mXPosInVolume) && BORDER_LOWY(this->mYPosInVolume) &&
      BORDER_HIGHZ(this->mZPosInVolume)) {
    return *(mCurrentVoxel - 1 - this->mVolume->getWidth() +
             this->mVolume->getWidth() * this->mVolume->getHeight());
  }
  return this->mVolume->getVoxelAt(this->mXPosInVolume - 1,
