...
namespace PolyVox {
template <>
POLYVOX_API void MeshDecimator<PositionMaterial>::fillInitialVertexMetadata(
    std::vector<InitialVertexMetadata>& vecVertexMetadata) {
    ...
    for (uint32_t ct = 0; ct < m_pOutputMesh->m_vecVertices.size(); ct++) {
        ...
        IntVertex intVertex(static_cast<uint32_t>(floatPos.getX()), static_cast<uint32_t>(floatPos.getY()),
                            static_cast<uint32_t>(floatPos.getZ()), ct);
    ...
    // Compute an approcimation to the normal, used when deciding if an edge can
    // collapse.
    for (uint32_t ct = 0; ct < m_pOutputMesh->m_vecVertices.size(); ct++) {
        Vector3DFloat sumOfNormals(0.0f, 0.0f, 0.0f);
        for (vector<uint32_t>::iterator iter = trianglesUsingVertex[ct].begin(); iter != trianglesUsingVertex[ct].end();
             iter++) {
    ...
    // Identify those vertices on the edge of a region. Care will need to be taken
    // when moving them.
    for (uint32_t ct = 0; ct < vecVertexMetadata.size(); ct++) {
        ...
        // Plus and minus X
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_NEG_X,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getX() <
                                                     regTransformed.getLowerCorner().getX() + 0.001f);
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_POS_X,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getX() >
                                                     regTransformed.getUpperCorner().getX() - 0.001f);
        // Plus and minus Y
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_NEG_Y,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getY() <
                                                     regTransformed.getLowerCorner().getY() + 0.001f);
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_POS_Y,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getY() >
                                                     regTransformed.getUpperCorner().getY() - 0.001f);
        // Plus and minus Z
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_NEG_Z,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getZ() <
                                                     regTransformed.getLowerCorner().getZ() + 0.001f);
        vecVertexMetadata[ct].isOnRegionFace.set(RFF_ON_REGION_FACE_POS_Z,
                                                 m_pOutputMesh->m_vecVertices[ct].getPosition().getZ() >
                                                     regTransformed.getUpperCorner().getZ() - 0.001f);
    }
}

template <>
POLYVOX_API void MeshDecimator<PositionMaterialNormal>::fillInitialVertexMetadata(
    std::vector<InitialVertexMetadata>& vecVertexMetadata) {
    vecVertexMetadata.clear();
    vecVertexMetadata.resize(m_pOutputMesh->m_vecVertices.size());
    ...
    // If all three vertices have the same material then we are not on a material
    // edge. If any vertex has a different material then all three vertices are on
    // a material edge. E.g. If one vertex has material 'a' and the other two have
    // material 'b', then the two 'b's are still on an edge (with 'a') even though
    // they are the same as eachother.
    for (uint32_t ct = 0; ct < m_vecTriangles.size(); ct++) {
        ...
        bool allMatch = (m_pOutputMesh->m_vecVertices[v0].material - m_pOutputMesh->m_vecVertices[v1].material < 0.001f) &&
...
POLYVOX_API bool MeshDecimator<PositionMaterial>::canCollapseNormalEdge(uint32_t uSrc, uint32_t uDst) {
    ...
    // It's not sufficient to just check the normals, there can be holes in the
    // middle of the mesh for example.
