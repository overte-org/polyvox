...
POLYVOX_API void MeshDecimator<PositionMaterial>::fillInitialVertexMetadata(
  ...
  for (uint32_t ct = 0; ct < m_pOutputMesh->m_vecVertices.size(); ct++) {
    ...
    IntVertex intVertex(static_cast<uint32_t>(floatPos.getX()),
                        static_cast<uint32_t>(floatPos.getY()),
  ...
  for (uint32_t ct = 0; ct < m_pOutputMesh->m_vecVertices.size(); ct++) {
    ...
    for (vector<uint32_t>::iterator iter = trianglesUsingVertex[ct].begin();
         iter != trianglesUsingVertex[ct].end(); iter++) {
  ...
  for (uint32_t ct = 0; ct < vecVertexMetadata.size(); ct++) {
    ...
    regTransformed.shift(regTransformed.getLowerCorner() *
                         static_cast<int32_t>(-1));

    // Plus and minus X
    vecVertexMetadata[ct].isOnRegionFace.set(
...
template <>
POLYVOX_API void
MeshDecimator<PositionMaterialNormal>::fillInitialVertexMetadata(
    std::vector<InitialVertexMetadata> &vecVertexMetadata) {
  vecVertexMetadata.clear();
  vecVertexMetadata.resize(m_pOutputMesh->m_vecVertices.size());
  ...
  for (uint32_t ct = 0; ct < m_vecTriangles.size(); ct++) {
    ...
    bool allMatch = (m_pOutputMesh->m_vecVertices[v0].material -
                         m_pOutputMesh->m_vecVertices[v1].material <
                     0.001f) &&
                    (m_pOutputMesh->m_vecVertices[v1].material -
                         m_pOutputMesh->m_vecVertices[v2].material <
                     0.001f);
...
template <>
POLYVOX_API bool
MeshDecimator<PositionMaterialNormal>::canCollapseNormalEdge(uint32_t uSrc,
                                                             uint32_t uDst) {
  if (m_vecInitialVertexMetadata[uSrc].normal.dot(
          m_vecInitialVertexMetadata[uDst].normal) <
...
template <>
POLYVOX_API bool
MeshDecimator<PositionMaterial>::canCollapseNormalEdge(uint32_t uSrc,
                                                       uint32_t uDst) {
