...
namespace PolyVox {
// Note: we don't do much error handling in here - exceptions will simply be
// propergated up to the caller.
// FIXME - think about pointer ownership issues. Or could return volume by value
// if the copy constructor is shallow
template <typename VolumeType>
polyvox_shared_ptr<VolumeType>
loadVolumeRaw(std::istream &stream,
              VolumeSerializationProgressListener *progressListener) {
  assert(false); // THIS FUNCTION IS DEPRECATED. REMOVE THIS ASSERT TO CONTINUE,
                 // BUT SWITCH TO 'loadVolume()' ASAP.

  // Read volume dimensions
  uint8_t volumeWidthPower = 0;
  uint8_t volumeHeightPower = 0;
  uint8_t volumeDepthPower = 0;
  stream.read(reinterpret_cast<char *>(&volumeWidthPower),
              sizeof(volumeWidthPower));
  stream.read(reinterpret_cast<char *>(&volumeHeightPower),
              sizeof(volumeHeightPower));
  stream.read(reinterpret_cast<char *>(&volumeDepthPower),
  ...
  polyvox_shared_ptr<VolumeType> volume(new LargeVolume<VolumeType::VoxelType>(
...
template <typename VolumeType>
void saveVolumeRaw(std::ostream &stream, VolumeType &volume,
                   VolumeSerializationProgressListener *progressListener) {
  assert(false); // THIS FUNCTION IS DEPRECATED. REMOVE THIS ASSERT TO CONTINUE,
                 // BUT SWITCH TO 'saveVolume()' ASAP.
  ...
  stream.write(reinterpret_cast<char *>(&volumeWidthPower),
               sizeof(volumeWidthPower));
  stream.write(reinterpret_cast<char *>(&volumeHeightPower),
               sizeof(volumeHeightPower));
  stream.write(reinterpret_cast<char *>(&volumeDepthPower),
               sizeof(volumeDepthPower));
...
template <typename VolumeType>
void saveVolumeRle(std::ostream &stream, VolumeType &volume,
                   VolumeSerializationProgressListener *progressListener) {
  assert(false); // THIS FUNCTION IS DEPRECATED. REMOVE THIS ASSERT TO CONTINUE,
                 // BUT SWITCH TO 'saveVolume()' ASAP.
      ...
      for (uint16_t x = 0; x < volumeWidth; ++x) {
        ...
        if (firstTime) {
          ...
          if (value == current) {
            ...
            stream.write(reinterpret_cast<char *>(&runLength),
                         sizeof(runLength));
            current = value;
            runLength = 1;
          }
...
template <typename VolumeType>
bool loadVolume(std::istream &stream, VolumeType &volume,
                VolumeSerializationProgressListener *progressListener) {
  char pIdentifier[8];
...
template <typename VolumeType>
bool saveVolume(std::ostream &stream, VolumeType &volume,
                VolumeSerializationProgressListener *progressListener) {
...
// if the copy constructor is shallow
template <typename VolumeType>
bool loadVersion0(std::istream &stream, VolumeType &volume,
                  VolumeSerializationProgressListener *progressListener) {
  // Read volume dimensions
  ...
  // Resize the volume
  // HACK - Forces block size to 32. This functions needs reworking anyway due
  // to large volume support.
  volume.resize(
      Region(Vector3DInt32(0, 0, 0),
             Vector3DInt32(volumeWidth - 1, volumeHeight - 1, volumeDepth - 1)),
      32);
...
template <typename VolumeType>
bool saveVersion0(std::ostream &stream, VolumeType &volume,
                  VolumeSerializationProgressListener *progressListener) {
  // Write volume dimensions
  uint16_t volumeWidth = volume.getWidth();
  uint16_t volumeHeight = volume.getHeight();
