...
OpenGLWidget::OpenGLWidget(QWidget *parent) : QGLWidget(parent), m_volData(0) {
  m_xRotation = 0;
  m_yRotation = 0;
...
void OpenGLWidget::setVolume(
    PolyVox::LargeVolume<MaterialDensityPair44> *volData) {
  ...
  if (m_volData != 0) {
    ...
    // Our volume is broken down into cuboid regions, and we create one mesh for
    // each region. This three-level for loop iterates over each region.
    for (uint16_t uRegionZ = 0; uRegionZ < m_uVolumeDepthInRegions;
         ++uRegionZ) {
      std::cout << "uRegionZ = " << uRegionZ << " of "
                << m_uVolumeDepthInRegions << std::endl;
      for (uint16_t uRegionY = 0; uRegionY < m_uVolumeHeightInRegions;
           ++uRegionY) {
        for (uint16_t uRegionX = 0; uRegionX < m_uVolumeWidthInRegions;
             ++uRegionX) {
          ...
          Vector3DInt32 regLowerCorner(regionStartX, regionStartY,
                                       regionStartZ);
          Vector3DInt32 regUpperCorner(regionEndX, regionEndY, regionEndZ);

          // Extract the surface for this region
          // extractSurface(m_volData, 0, PolyVox::Region(regLowerCorner,
          // regUpperCorner), meshCurrent);

          polyvox_shared_ptr<SurfaceMesh<PositionMaterialNormal>> mesh(
              new SurfaceMesh<PositionMaterialNormal>);
          MarchingCubesSurfaceExtractor<LargeVolume<MaterialDensityPair44>>
              surfaceExtractor(volData,
                               PolyVox::Region(regLowerCorner, regUpperCorner),
                               mesh.get());
          surfaceExtractor.execute();

          // decimatedMesh->generateAveragedFaceNormals(true);

          // computeNormalsForVertices(m_volData, *(decimatedMesh.get()),
          // SOBEL_SMOOTHED); *meshCurrent = getSmoothedSurface(*meshCurrent);
          // mesh->smooth(0.3f);
          ...
          if (mesh->m_vecTriangleIndices.size() > 0) {
            ...
            if (m_bUseOpenGLVertexBufferObjects) {
              OpenGLSurfaceMesh openGLSurfaceMesh =
                  BuildOpenGLSurfaceMesh(*(mesh.get()));
              m_mapOpenGLSurfaceMeshes.insert(
                  make_pair(v3dRegPos, openGLSurfaceMesh));
            }
            // else
            //{
    ...
    // Projection matrix is dependant on volume size, so we need to set it up
    // again.
...
void OpenGLWidget::paintGL() {
  if (m_volData) {
    ...
    glTranslatef(-g_uVolumeSideLength / 2, -g_uVolumeSideLength / 2,
                 -g_uVolumeSideLength / 2);
      ...
      for (uint16_t uRegionY = 0; uRegionY < m_uVolumeHeightInRegions;
        ...
        for (uint16_t uRegionX = 0; uRegionX < m_uVolumeWidthInRegions;
          ...
          if (m_mapSurfaceMeshes.find(v3dRegPos) != m_mapSurfaceMeshes.end()) {
            polyvox_shared_ptr<SurfaceMesh<PositionMaterialNormal>>
                meshCurrent = m_mapSurfaceMeshes[v3dRegPos];
            unsigned int uLodLevel =
                0; // meshCurrent->m_vecLodRecords.size() - 1;
            if (m_bUseOpenGLVertexBufferObjects) {
              renderRegionVertexBufferObject(
                  m_mapOpenGLSurfaceMeshes[v3dRegPos], uLodLevel);
            } else {
