...
OpenGLSurfaceMesh
BuildOpenGLSurfaceMesh(const SurfaceMesh<PositionMaterialNormal> &mesh) {
  ...
  glBufferData(GL_ARRAY_BUFFER, vecVertices.size() * sizeof(GLfloat) * 9, 0,
               GL_STATIC_DRAW);
  GLfloat *ptr = (GLfloat *)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_WRITE);

  for (vector<PositionMaterialNormal>::const_iterator iterVertex =
           vecVertices.begin();
       iterVertex != vecVertices.end(); ++iterVertex) {
...
void renderRegionVertexBufferObject(const OpenGLSurfaceMesh &openGLSurfaceMesh,
  ...
  int beginIndex =
      openGLSurfaceMesh.sourceMesh->m_vecLodRecords[uLodLevel].beginIndex;
  int endIndex =
      openGLSurfaceMesh.sourceMesh->m_vecLodRecords[uLodLevel].endIndex;
  ...
  // glDrawElements(GL_TRIANGLES, openGLSurfaceMesh.noOfIndices,
  // GL_UNSIGNED_INT, 0);
  glDrawRangeElements(GL_TRIANGLES, beginIndex, endIndex - 1,
                      endIndex - beginIndex,
                      /* openGLSurfaceMesh.noOfIndices,*/ GL_UNSIGNED_INT, 0);
