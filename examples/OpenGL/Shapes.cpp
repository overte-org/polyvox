...
void createSphereInVolume(LargeVolume<MaterialDensityPair44>& volData, float fRadius, uint8_t uValue) {
    ...
    Vector3DInt32 v3dVolCenter =
        (volData.getEnclosingRegion().getUpperCorner() - volData.getEnclosingRegion().getLowerCorner()) /
        ...
        for (int y = 0; y < volData.getHeight(); y++) {
            for (int x = 0; x < volData.getWidth(); x++) {
                ...
                if (fDistToCenter <= fRadius) {
                    volData.setVoxelAt(x, y, z,
                                       MaterialDensityPair44(uValue, uValue > 0 ? MaterialDensityPair44::getMaxDensity()
...
void createCubeInVolume(LargeVolume<MaterialDensityPair44>& volData,
                        Vector3DInt32 lowerCorner,
                        Vector3DInt32 upperCorner,
                        uint8_t uValue) {
