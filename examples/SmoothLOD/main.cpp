  ...
  for (int z = 0; z < volData.getDepth(); z++) {
    for (int y = 0; y < volData.getHeight(); y++) {
      for (int x = 0; x < volData.getWidth(); x++) {
        ...
        // 144 in the middle, (144 - 32) at the edges. Threshold of 128 is
        // between these volData.setVoxelAt(x, y, z, 144 - fDistToCenter);
      }
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // smoothRegion<SimpleVolume, Density8>(volData,
  // volData.getEnclosingRegion()); smoothRegion<SimpleVolume,
  // Density8>(volData, volData.getEnclosingRegion());
  // smoothRegion<SimpleVolume, Density8>(volData,
  // volData.getEnclosingRegion());

  RawVolume<uint8_t> volDataLowLOD(
      PolyVox::Region(Vector3DInt32(0, 0, 0), Vector3DInt32(15, 31, 31)));

  VolumeResampler<SimpleVolume<uint8_t>, RawVolume<uint8_t>> volumeResampler(
      &volData,
      PolyVox::Region(Vector3DInt32(0, 0, 0), Vector3DInt32(31, 63, 63)),
      &volDataLowLOD, volDataLowLOD.getEnclosingRegion());
  volumeResampler.execute();

  // Extract the surface
  SurfaceMesh<PositionMaterialNormal> meshLowLOD;
  MarchingCubesSurfaceExtractor<RawVolume<uint8_t>> surfaceExtractor(
      &volDataLowLOD, volDataLowLOD.getEnclosingRegion(), &meshLowLOD);
  surfaceExtractor.execute();
  meshLowLOD.scaleVertices(/*2.0f*/ 63.0f / 31.0f);

  // Extract the surface
  SurfaceMesh<PositionMaterialNormal> meshHighLOD;
  MarchingCubesSurfaceExtractor<SimpleVolume<uint8_t>> surfaceExtractorHigh(
      &volData,
      PolyVox::Region(Vector3DInt32(30, 0, 0), Vector3DInt32(63, 63, 63)),
      &meshHighLOD);
